...
const asset = (state = initialState, action) => {
  switch (action.type) {
    ...
    case GET_ASSET_HISTORY:
      const data = action.payload.newAssetHistory.map((item) => {
        return {
          key: item.id,
          assigned_date: formatDate(item.assign_date),
          assigned_to: item.to_username,
          assigned_by: item.by_username,
          returned_date: formatDate(item.returned_date),
        }
      })
      return {
        ...state,
        newAssetHistory: data,
        code: action.payload.code,
      }
